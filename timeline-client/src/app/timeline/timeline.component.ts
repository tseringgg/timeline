...
export class TimelineComponent implements OnInit, AfterViewInit {
  ...
  getEventTextPosition(era: string, event:EventViewModel, eventCount: number, index:number, textIndex: number): object {
    ...
    if (event.countryName.toLocaleLowerCase() === 'tibet') {
      leftPos = this.getTibetEventTextLeftPos(); // '20%'; //'15em';
    } else {
      leftPos = '55%'; //'50em';
    }
  ...
  getTibetEventTextLeftPos(): string {
    let leftPos: string;
    if (this.windowWidth > 992) leftPos = '20%';
    if (this.windowWidth <= 992) leftPos = '15%';
    if (this.windowWidth <= 768) leftPos = '10%';
    if (this.windowWidth <= 600) leftPos = '8%';
    if (this.windowWidth <= 400) leftPos = '3%';
    return leftPos;
  }
